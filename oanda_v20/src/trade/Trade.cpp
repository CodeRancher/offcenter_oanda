...
void to_json(nlohmann::json& j, const Trade& p) {
	j = nlohmann::json {
		{"id", p.id},
		...
		{"takeProfitOrder", p.takeProfitOrder},
		{"stopLossOrder", p.stopLossOrder},
		{"trailingStopLossOrder", p.trailingStopLossOrder}
	};
}

void from_json(const nlohmann::json& j, Trade& p) {
	try {
		...
		oanda::v20::json::convertJson(j, "takeProfitOrder", p.takeProfitOrder);
		oanda::v20::json::convertJson(j, "stopLossOrder", p.stopLossOrder);
		oanda::v20::json::convertJson(j, "trailingStopLossOrder", p.trailingStopLossOrder);
