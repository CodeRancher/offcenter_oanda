...
void to_json(nlohmann::json& j, const PriceBucket& p) {
	j = nlohmann::json {
		{"price", p.price},
...
void from_json(const nlohmann::json& j, PriceBucket& p) {
	try {
		oanda::v20::json::convertJson(j, "price", p.price);
		oanda::v20::json::convertJson(j, "liquidity", p.liquidity);
