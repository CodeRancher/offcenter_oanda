...
/**
 * File: MarginCallExtendTransaction.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const MarginCallExtendTransaction& p) {
	j = nlohmann::json {
		{"id", p.id},
		{"time", p.time},
		{"userID", p.userID},
...
void from_json(const nlohmann::json& j, MarginCallExtendTransaction& p) {
	try {
		...
		oanda::v20::json::convertJson(j, "extensionNumber", p.extensionNumber);
