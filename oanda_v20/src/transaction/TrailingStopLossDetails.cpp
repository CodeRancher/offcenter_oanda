...
/**
 * File: TrailingStopLossDetails.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const TrailingStopLossDetails& p) {
	j = nlohmann::json {
		{"distance", p.distance},
...
void from_json(const nlohmann::json& j, TrailingStopLossDetails& p) {
	try {
