...
/**
 * File: DailyFinancingTransaction.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const DailyFinancingTransaction& p) {
	j = nlohmann::json {
		{"id", p.id},
		{"time", p.time},
		{"userID", p.userID},
...
void from_json(const nlohmann::json& j, DailyFinancingTransaction& p) {
	try {
		...
		oanda::v20::json::convertJson(j, "positionFinancings", p.positionFinancings);
