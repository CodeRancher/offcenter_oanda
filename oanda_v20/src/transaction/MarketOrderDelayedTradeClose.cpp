...
/**
 * File: MarketOrderDelayedTradeClose.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const MarketOrderDelayedTradeClose& p) {
	j = nlohmann::json {
		{"tradeID", p.tradeID},
...
void from_json(const nlohmann::json& j, MarketOrderDelayedTradeClose& p) {
	try {
		oanda::v20::json::convertJson(j, "tradeID", p.tradeID);
		oanda::v20::json::convertJson(j, "clientTradeID", p.clientTradeID);
		oanda::v20::json::convertJson(j, "sourceTransactionID", p.sourceTransactionID);
