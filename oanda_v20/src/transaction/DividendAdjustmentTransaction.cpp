...
/**
 * File: DividendAdjustmentTransaction.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const DividendAdjustmentTransaction& p) {
	j = nlohmann::json {
		{"id", p.id},
		{"time", p.time},
		{"userID", p.userID},
...
void from_json(const nlohmann::json& j, DividendAdjustmentTransaction& p) {
	try {
		...
		oanda::v20::json::convertJson(j, "quoteDividendAdjustment", p.quoteDividendAdjustment);
		...
		oanda::v20::json::convertJson(j, "openTradeDividendAdjustments", p.openTradeDividendAdjustments);
