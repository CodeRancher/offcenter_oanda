...
/**
 * File: DelayedTradeClosureTransaction.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const DelayedTradeClosureTransaction& p) {
	j = nlohmann::json {
		{"id", p.id},
		{"time", p.time},
		{"userID", p.userID},
...
void from_json(const nlohmann::json& j, DelayedTradeClosureTransaction& p) {
	try {
