...
/**
 * File: OrderClientExtensionsModifyRejectTransaction.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const OrderClientExtensionsModifyRejectTransaction& p) {
	j = nlohmann::json {
		{"id", p.id},
		{"time", p.time},
		{"userID", p.userID},
...
void from_json(const nlohmann::json& j, OrderClientExtensionsModifyRejectTransaction& p) {
	try {
		...
		oanda::v20::json::convertJson(j, "clientExtensionsModify", p.clientExtensionsModify);
