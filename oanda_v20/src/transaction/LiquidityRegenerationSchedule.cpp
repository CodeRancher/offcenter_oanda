...
/**
 * File: LiquidityRegenerationSchedule.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const LiquidityRegenerationSchedule& p) {
	j = nlohmann::json {
		{"steps", p.steps}
	};
}

void from_json(const nlohmann::json& j, LiquidityRegenerationSchedule& p) {
	try {
		oanda::v20::json::convertJson(j, "steps", p.steps);
