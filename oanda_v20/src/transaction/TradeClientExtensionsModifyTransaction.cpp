...
/**
 * File: TradeClientExtensionsModifyTransaction.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const TradeClientExtensionsModifyTransaction& p) {
	j = nlohmann::json {
		{"id", p.id},
		{"time", p.time},
		{"userID", p.userID},
...
void from_json(const nlohmann::json& j, TradeClientExtensionsModifyTransaction& p) {
	try {
