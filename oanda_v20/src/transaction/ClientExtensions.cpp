...
void to_json(nlohmann::json& j, const ClientExtensions& p) {
	j = nlohmann::json {
		{"id", p.id},
		{"tag", p.tag},
		{"comment", p.comment}
...
void from_json(const nlohmann::json& j, ClientExtensions& p) {
	try {
		oanda::v20::json::convertJson(j, "id", p.id);
		oanda::v20::json::convertJson(j, "tag", p.tag);
