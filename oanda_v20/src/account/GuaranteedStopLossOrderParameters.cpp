...
/**
 * File: GuaranteedStopLossOrderParameters.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const GuaranteedStopLossOrderParameters& p) {
	j = nlohmann::json {
		{"mutabilityMarketOpen", p.mutabilityMarketOpen},
		{"mutabilityMarketHalted", p.mutabilityMarketHalted}
	};
}

void from_json(const nlohmann::json& j, GuaranteedStopLossOrderParameters& p) {
	try {
		oanda::v20::json::convertJson(j, "mutabilityMarketOpen", p.mutabilityMarketOpen);
		oanda::v20::json::convertJson(j, "mutabilityMarketHalted", p.mutabilityMarketHalted);
