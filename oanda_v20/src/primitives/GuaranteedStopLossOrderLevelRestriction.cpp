...
/**
 * File: GuaranteedStopLossOrderLevelRestriction.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const GuaranteedStopLossOrderLevelRestriction& p) {
	j = nlohmann::json {
		{"volume", p.volume},
...
void from_json(const nlohmann::json& j, GuaranteedStopLossOrderLevelRestriction& p) {
	try {
		oanda::v20::json::convertJson(j, "volume", p.volume);
		oanda::v20::json::convertJson(j, "priceRange", p.priceRange);
