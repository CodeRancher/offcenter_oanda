...
void to_json(nlohmann::json& j, const ConversionFactor& p) {
	j = nlohmann::json {
		{"factor", p.factor}
	};
}

void from_json(const nlohmann::json& j, ConversionFactor& p) {
	try {
		oanda::v20::json::convertJson(j, "factor", p.factor);
