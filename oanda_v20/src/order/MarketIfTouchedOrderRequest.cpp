...
/**
 * File: MarketIfTouchedOrderRequest.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const MarketIfTouchedOrderRequest& p) {
	j = nlohmann::json {
		{"type", p.type},
...
void from_json(const nlohmann::json& j, MarketIfTouchedOrderRequest& p) {
	try {
