...
/**
 * File: TakeProfitOrderRequest.cpp
 * Author: Scott Brauer
 *
...
void to_json(nlohmann::json& j, const TakeProfitOrderRequest& p) {
	j = nlohmann::json {
		{"type", p.type},
		{"tradeID", p.tradeID},
...
void from_json(const nlohmann::json& j, TakeProfitOrderRequest& p) {
	try {
