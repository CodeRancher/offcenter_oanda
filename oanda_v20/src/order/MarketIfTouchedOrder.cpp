...
void to_json(nlohmann::json& j, const MarketIfTouchedOrder& p) {
	j = nlohmann::json {
		{"id", p.id},
...
void from_json(const nlohmann::json& j, MarketIfTouchedOrder& p) {
	try {
		...
		oanda::v20::json::convertJson(j, "initialMarketPrice", p.initialMarketPrice);
