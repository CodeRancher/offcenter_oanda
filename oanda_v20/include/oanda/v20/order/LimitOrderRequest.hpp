...
/**
 * @file   LimitOrderRequest.hpp
 * @author Scott Brauer
 * @date   12-07-2020  
 */

#ifndef OANDA_V20_ORDER_LIMITORDERREQUEST_HPP
#define OANDA_V20_ORDER_LIMITORDERREQUEST_HPP
...
/**
 * @brief A LimitOrderRequest specifies the parameters that may be set when creating a Limit Order.
 *
 * LimitOrderRequest is an application/json object with the following Schema:
 */
struct LimitOrderRequest {
	LimitOrderRequest():
		type(oanda::v20::order::OrderType::LIMIT),
	...
	/**
	 * @brief The type of the Order to Create. Must be set to “LIMIT” when creating a Market Order.
	 */
	oanda::v20::order::OrderType type;

	/**
	 * @brief The Limit Order’s Instrument.
	 */
	oanda::v20::primitives::InstrumentName instrument;

	/**
	 * @brief The quantity requested to be filled by the Limit Order.
	 *
	 * A positive number of units results in a long Order, and a negative number of units results
	 * in a short Order.
	 */
	oanda::v20::primitives::DecimalNumber units;

	/**
	 * @brief The price threshold specified for the Limit Order.
	 *
	...
	/**
	 * @brief The time-in-force requested for the Limit Order.
	 */
	oanda::v20::order::TimeInForce timeInForce;

	/**
	 * @brief The date/time when the Limit Order will be cancelled if its timeInForce is “GTD”.
	 */
	...
	/**
	 * @brief GuaranteedStopLossDetails specifies the details of a Guaranteed Stop Loss Order to be created on behalf of a client.
	 *
	 * This may happen when an Order
...
#endif /* OANDA_V20_ORDER_LIMITORDERREQUEST_HPP */
